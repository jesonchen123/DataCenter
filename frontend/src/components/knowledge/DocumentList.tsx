...
const STATUS_STYLE: Record<string, { bg: string; color: string; border: string; label: string }> = {
  ...
  archived: { bg: '#f5f5f5', color: '#999', border: '#e0e0e0', label: '已归档' },
}
...
interface DocumentListProps {
  ...
  onScenarioFilterChange: (v: string) => void
    ...
    <section className="flex-shrink-0 flex flex-col bg-white border-r border-gray-200 h-full" style={{ width: '45%' }}>
      ...
      <div className="flex-shrink-0 px-5 pt-5 pb-3 border-b border-gray-100">
        <div className="flex items-center justify-between mb-3">
          ...
          {total > 0 && (
            <span
              ...
              style={{ width: 24, height: 24, fontSize: 12, background: '#0066CC', color: '#fff' }}
            >
              {total}
            </span>
          )}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <FilterChip label="全部风险" value="" current={riskFilter} onChange={onRiskFilterChange} />
          <FilterChip
            label="高风险"
            value="high"
            current={riskFilter}
