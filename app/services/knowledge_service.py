...
def generate_knowledge_doc(segment: dict) -> dict:
    ...
    scenario_type = str(segment.get("scenario_type") or "other").strip()

    if contains_price_info:
        title = "客户咨询价格时的标准回复"
        doc_content = _qa_content(customer_question or "这个产品多少钱？", "具体价格以公司正式报价为准。")
        tags = _build_tags(scenario_type, ["价格咨询", "售前", "需人工确认"])
        ...
        tags = _build_tags(scenario_type, ["产品咨询"])
...
def _build_tags(scenario_type: str, base_tags: list[str]) -> list[str]:
    if scenario_type and scenario_type != "other":
        return [scenario_type] + base_tags
    return base_tags
